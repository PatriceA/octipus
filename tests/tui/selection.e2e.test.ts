...
describe.skipIf(process.platform === 'win32')('terminal selection and copy', () => {
  test.each(['src/tui-pi/index.ts', 'src/tui-editor/index.ts'])('%s starts with native selection, opts into wheel capture, and keeps copy and keyboard history usable', async entry => {
    ...
    tui.send('\x1bm'); await tui.waitFor('[wheel captured');
    ...
    expect(tui.screen.modes.mouseTrackingMode).toBe('none');
    tui.send('\x1b[1;5F'); // Ctrl+End: show notices before the clipboard request.
    ...
    tui.send('\x1bm');
    // Capture changes while history stays frozen without submitting the draft.
