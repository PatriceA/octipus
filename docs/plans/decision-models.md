...
| 6 | **Router LLM classifier**, `src/core/router.ts:184` `classifyWithLLM` | Free-text LLM on the default model | `choice` agent/intent. Reachable only through `route()` (`agent-manager.ts:154`, `api/routes/agents.ts:483`), so **check reachability first** before touching it. | personal |
...
| P5 | Local System-One provider once one exists (open weights, Ollama/llama.cpp) | `decide()` implementation only, no site changes; `personal` sites run fully local |
