...
`scripts/**/*.test.ts`). Requires Postgres + Redis for some specs — most run
...
Previously-skipped integration tests backed by `docker-compose.test.yml`. Spins up an isolated Postgres + Redis and runs specs that need real DB transactions or cross-process pub/sub. Coverage includes MCP transports (stdio + SSE) and storage provider parity (Redis vs in-memory).
