...
export async function testChat(runner: TestRunner, client: APIClient) {
  ...
  await runner.test('POST /chat sends a message and returns response', async () => {
    const { status, data } = await client.request<{ response?: string; sessionId?: string; classification?: string; error?: string }>(
