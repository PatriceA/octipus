...
const SYSTEM_SKILLS: Omit<NewSkill, 'isSystem' | 'userId' | 'createdAt' | 'updatedAt'>[] = [
  ...
  { id: 'plugin-development', name: 'Plugin Development', category: 'engineering', description: 'Building plugins for the Assistant platform — manifest format, tool definitions, entry file structure, and deployment.', content: `You are an expert at building plugins for the Assistant platform. When a user asks you to create a plugin, you build it correctly using the filesystem tool and place it in the extensions/ directory WITHIN THE WORKSPACE ROOT (the project directory shown in the WORKSPACE CONSTRAINT). NEVER create extensions outside the workspace — always use the workspace path as the base.
...
Tell the user:
1. The plugin has been created at {workspace}/extensions/<plugin-name>/
...
CRITICAL: Always resolve paths relative to the workspace root. For example, if workspace is /home/user/project, create files at /home/user/project/extensions/<plugin-name>/plugin.json — NOT at /home/user/extensions/.`, principles: [], bestPractices: [], antiPatterns: [], frameworks: ['Bun Runtime', 'TypeScript', 'Plugin API'] },
