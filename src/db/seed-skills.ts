...
const SYSTEM_SKILLS: Omit<NewSkill, 'isSystem' | 'userId' | 'createdAt' | 'updatedAt'>[] = [
  ...
  { id: 'plugin-development', name: 'Plugin Development', category: 'coding', description: 'Building plugins for the Assistant platform — manifest format, tool definitions, entry file structure, and deployment.', content: `You are an expert at building plugins for the Assistant platform. When a user asks you to create a plugin, you build it correctly using the filesystem tool and place it in the extensions/ directory inside ASSISTANT_HOME (shown in the system prompt). NEVER create extensions in the workspace root — always use the ASSISTANT_HOME path as the base for plugins.
...
Tell the user:
1. The plugin has been created at ASSISTANT_HOME/extensions/<plugin-name>/
...
CRITICAL: Always use the ASSISTANT_HOME path from the system prompt for plugin paths. The workspace root may be a different directory (e.g., a parent folder). Plugins MUST go in ASSISTANT_HOME/extensions/, not workspace/extensions/.`, principles: [], bestPractices: [], antiPatterns: [], frameworks: ['Bun Runtime', 'TypeScript', 'Plugin API'] },
