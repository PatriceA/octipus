 ...
 * the app appears here too. This route only owns auth + the DB/storage probes.
 */

/** Resolve a probe to `0` if it rejects OR exceeds `ms` — a scrape must never
 *  block on a wedged dependency, and unit tests have no live DB/storage. */
...
async function collectExposition(): Promise<string> {
  const [db, storageUp] = await Promise.all([
  ...
  return renderMetrics(db, storageUp);
}
