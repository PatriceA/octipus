...
// MOONSHOT_BASE_URL. https://platform.kimi.ai/docs/overview
const MOONSHOT_BASE_URL = 'https://api.moonshot.ai/v1';

// Kimi reasoning variants (kimi-thinking, k1.5) can stream for minutes.
const REASONING_TIMEOUT_MS = 1_800_000; // 30 min
const DEFAULT_TIMEOUT_MS = 120_000;

/** Detect Kimi reasoning variants by id. Conservative id-substring match. */
function isMoonshotReasoningModel(modelName: string): boolean {
  const lower = (modelName || '').toLowerCase();
  return /thinking|k1\.5|reasoning/.test(lower);
}

/**
 * Moonshot (Kimi) direct provider — calls the Moonshot OpenAI-compatible API
 * without going through the LiteLLM proxy. Context caching is automatic on a
 * stable prefix; cached prompt tokens arrive under
 * `usage.prompt_tokens_details.cached_tokens` (captured by extractCachedTokens).
 * API key is retrieved from the vault at runtime (env var takes precedence).
 */
export class MoonshotProvider implements ModelProvider {
  ...
  constructor(baseUrl?: string) {
    this.baseUrl = baseUrl || process.env.MOONSHOT_BASE_URL || MOONSHOT_BASE_URL;
  }
  ...
  async complete(options: CompletionOptions): Promise<CompletionResult> {
    ...
    modelLogger.debug(
      ...
      'Sending completion request to Moonshot'
    );
    ...
    try {
      ...
      modelLogger.debug(
        ...
        'Moonshot completion successful'
      );

      return result;
    } catch (error) {
      modelLogger.error({ error, model: params.model, provider: this.name }, 'Moonshot completion failed');
  ...
  async *stream(options: CompletionOptions): AsyncGenerator<StreamChunk> {
    ...
    modelLogger.debug({ model: params.model, provider: this.name }, 'Starting streaming completion via Moonshot');
  ...
  async checkHealth(): Promise<ProviderHealthStatus> {
    ...
    try {
      ...
      if (!apiKey) {
        return { healthy: false, error: 'Moonshot API key not configured' };
      }
  ...
  /**
   * Live-list models from the Moonshot API (OpenAI-compatible `/models`).
   * Throws if no API key is configured or the upstream call fails.
   */
  ...
  private async getApiKey(): Promise<string | null> {
    ...
    try {
      ...
      const value = await vault.getByName('system', 'moonshot_api_key');
      return value || null;
    } catch (err) {
      modelLogger.warn({ err: (err as Error).message, provider: this.name }, 'Moonshot vault lookup failed; falling back to env var');
      return null;
    }
  }
  ...
  private async createClient(modelName?: string): Promise<OpenAI> {
    const apiKey = await this.getApiKey();
    if (!apiKey) {
      throw classifyError(new Error('Moonshot API key not available. Set MOONSHOT_API_KEY or store it in the vault.'), this.name);
    }

    const timeout = modelName && isMoonshotReasoningModel(modelName)
