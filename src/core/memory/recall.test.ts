...
/**
 * `recordAccess` is fire-and-forget: `retrieveForContext` does not await it, by
 * design. Let it land before the next test reads or restores access counts —
 * otherwise a write from the previous test arrives mid-comparison and reorders
 * a standing ranking the test is not varying.
 */
afterEach(async () => {
