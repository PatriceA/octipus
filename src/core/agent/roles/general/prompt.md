...
3. Verify. If the task names a command (`python3 -m unittest discover`, `npm test`), run it once with `cwd` set. If it is a brief with no test suite, check the checklist with a few short `python3 -c` calls — several requirements per call, asserting each type, field order, sort order, message and exit code — and fix what fails. Do NOT write a test file unless the brief asks for one. Quote the exit code and a line of output.
