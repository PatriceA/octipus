    ...
    parameters: {
      ...
      properties: {
        ...
        scorers: {
          type: 'array',
          description:
            'Optional deterministic checks the child output MUST pass. Run after the child returns; any failure marks the result contract_failed and the child is re-dispatched with the failures quoted back to it (once, by default), so a check you can state is a defect you do not have to catch by reading. Kinds: {"kind":"non_empty"}, {"kind":"contains","value":"...","on":"output|notes"}, {"kind":"regex","pattern":"...","flags":"i","on":"output|notes"}, {"kind":"json","requiredKeys":["a","b"]}, {"kind":"file_exists","path":"report.md"}, {"kind":"side_effect","minFilesChanged":1}, {"kind":"command_exit_zero","command":"npm test","timeoutMs":120000}. Prefer command_exit_zero whenever "done" can be settled by running something (a suite, a build, a type-check): it is the only check that produces new evidence rather than reading what the child said. The command is run in the child\'s workspace with NO shell features — a plain argv, so no pipes, redirects, ;, &&, $() or backticks. Do NOT prefix a `cd`: the child already runs in the right directory, `cd` is a shell builtin with no binary to spawn, and `cd X && npm test` is rejected outright. Write the bare command: "npm test", "pytest -q", "python3 -m unittest discover". If a check needs a path, keep it relative to the workspace; an absolute path containing a space must be quoted, or argv splits it. When "done" could be proved more than one way — you do not know yet whether the child will write a pytest suite or a unittest one — state the alternatives instead of guessing: {"kind":"any_of","scorers":[{"kind":"command_exit_zero","command":"pytest -q"},{"kind":"command_exit_zero","command":"python3 -m unittest discover"}]} passes if ANY member passes, and costs nothing extra because evaluation stops at the first one that does. A single command that guesses wrong marks correct work as contract_failed.',
          items: { type: 'object' },
        },
      },
