...
/**
 * Web search skill — tries SearXNG first, falls back to DuckDuckGo via Playwright.
...
export class WebSearchSkill extends BaseSkill {
  ...
  private async search(args: Record<string, unknown>): Promise<unknown> {
    ...
    if (this.searxngAvailable !== false) {
      try {
        ...
        skillLogger.info('SearXNG unavailable, falling back to browser-based search');
      }
    }
  ...
  private async searchViaBrowser(query: string, maxResults: number): Promise<unknown> {
    ...
    try {
      ...
      skillLogger.warn({ error: (error as Error).message, query }, 'Google search failed, trying DuckDuckGo');
    }

    try {
      return await this.searchViaDuckDuckGo(query, maxResults);
    } catch (error) {
      skillLogger.error({ error, query }, 'All browser search methods failed');
  ...
  private async searchViaGoogle(query: string, maxResults: number): Promise<unknown> {
    ...
    try {
      ...
      skillLogger.debug({ query, resultCount: results.length, method: 'google' }, 'Search completed');
  ...
  private async searchViaDuckDuckGo(query: string, maxResults: number): Promise<unknown> {
    ...
    try {
      ...
      skillLogger.debug({ query, resultCount: results.length, method: 'duckduckgo' }, 'Search completed');
  ...
  private async fetchPage(args: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      ...
      skillLogger.debug({ url, textLength: trimmedText.length }, 'Page fetched via browser');
      ...
      skillLogger.error({ error, url }, 'Browser page fetch failed');
  ...
  private async getOrCreateBrowser(): Promise<Browser> {
    if (!this.browser || !this.browser.isConnected()) {
      ...
      skillLogger.info('Browser launched for web search');
    }
    return this.browser;
  }
  ...
  async shutdown(): Promise<void> {
    if (this.browser) {
      ...
      this.browser = null;
      skillLogger.info('Web search browser closed');
    }
  }
}
