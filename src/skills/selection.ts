...
export async function getSkillModes(userId: string, sessionId?: string): Promise<Map<string, SkillMode>> {
  ...
  for (const row of rows.filter(row => row.scope === '')) modes.set(registry.canonicalId(row.skillId), row.mode);
  for (const row of rows.filter(row => row.scope !== '')) modes.set(registry.canonicalId(row.skillId), row.mode);
  return modes;
}
...
export async function buildSelectedSkillPrompt(userId: string, sessionId?: string): Promise<string> {
  ...
  const registry = getSkillRegistry();
  const visible = new Set((await registry.getAll(userId)).map(skill => skill.id));
  ...
  const content = await registry.buildPromptFragment(ids, userId);
